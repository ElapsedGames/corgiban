...
3) ESLint with eslint-plugin-boundaries
   ...
   b) new Worker() placement (from WORKER_CREATION_RULE) - ESLint only, not dependency-cruiser:
      ...
      Use no-restricted-syntax with a scoped ESLint override:
        - Apply globally: flag NewExpression[callee.name='Worker'] and
          NewExpression[callee.name='SharedWorker'] as errors.
        - Override only for `*.client.ts` paths declared in WORKER_CREATION_RULE.allowedGlobs.
   ...
   c) Date/Date.now restriction for deterministic packages (packages/core/**, packages/solver/**):
      Both direct and globalThis forms must be caught:
        - no-restricted-globals (scoped override): ['Date']
...
- Include src/**/*.ts. Target: Node (moduleResolution: bundler or node16). Strict on. No emit.
...
CI gate (GitHub Actions, every PR) - no changes to existing gate needed:
...
Test pure functions directly (not primarily via subprocess). Tests live in tools/src/__tests__/.
